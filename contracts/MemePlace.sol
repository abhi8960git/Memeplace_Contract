...
function reSellToken(uint256 tokenId, uint256 _price) public payable{
    require(idMarketItem[tokenId].owner == msg.sender , "only item owner can perform this perchage" );
     require(msg.value == listingPrice, "Price must be equal to listing price");
...
function createMarketSale(uint256 tokenId)public payable {
    ...
    require(msg.value == price, "Please submit the asking price in order to complet the purchage ");
    idMarketItem[tokenId].seller = payable(msg.sender);
